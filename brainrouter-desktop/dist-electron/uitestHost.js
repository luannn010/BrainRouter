...
async function dispatch(layer, step) {
    switch (step.action) {
        ...
        default:
            return errorResult('unknown', `unknown action: ${String(step.action)}`);
    }
}
export function createUiTestHost(workspaceRoot) {
    // In-memory incremental state: re-parse only files whose hash changed.
    const fileSites = new Map();
    const hashes = new Map();
    let lastBroad; // extraction mode the cache was built in
    ...
    function extract(opts) {
        ...
        for (const f of files) {
            ...
            if (hashes.get(f.path) === h && fileSites.has(f.path))
    ...
    const flowsDir = () => path.join(workspaceRoot, '.brainrouter', 'ui-tests', 'flows');
    function listFlows() {
        try {
            return {
                flows: fs
    ...
    function saveFlow(name, steps) {
        try {
            ...
            const yaml = serializeFlowYaml({ name: safe, steps: steps });
    ...
    async function runFlow(arg) {
        ...
        if (arg.name) {
            try {
                steps = parseFlowYaml(fs.readFileSync(path.join(flowsDir(), `${sanitizeFlowName(arg.name)}.flow.yaml`), 'utf8')).steps;
            }
            catch {
        ...
        return { results: await runFlowSteps(session().layer, steps) };
    }
    // --- Stories (named user journeys) ---------------------------------------
    const storiesDir = () => path.join(workspaceRoot, '.brainrouter', 'ui-tests', 'stories');
    function listStories() {
        ...
        const readJson = (file) => {
            try {
                if (fs.existsSync(file))
                    for (const s of parseStoriesJson(fs.readFileSync(file, 'utf8')))
                        byId.set(s.id, s);
            }
        ...
        try {
            ...
            for (const f of fs.readdirSync(dir)) {
                if (!f.endsWith('.story.yaml'))
                    continue;
                try {
                    const s = parseStoryYaml(fs.readFileSync(path.join(dir, f), 'utf8'));
                    byId.set(s.id, s);
                }
                catch { /* skip a bad story */ }
            }
        }
    ...
    const screenshotsDir = () => path.join(workspaceRoot, '.brainrouter', 'ui-tests', 'screenshots');
    const reportsDir = () => path.join(workspaceRoot, '.brainrouter', 'ui-tests', 'reports');
    const relFromRoot = (abs) => path.relative(workspaceRoot, abs).split(path.sep).join('/');
    ...
    let devServer = null;
    function readLaunchConfigs() {
        try {
            const j = JSON.parse(fs.readFileSync(path.join(workspaceRoot, '.claude', 'launch.json'), 'utf8'));
    ...
    async function ensureApp(opts) {
        ...
        if (!(await isPortFree(cfg.port))) {
            ...
            if (!alt)
                return { url, started: false, error: `port ${cfg.port} is in use and no free port was found near it` };
        ...
        if (!ready) {
            stopDevServer();
            return { url: spawnUrl, started: false, error: `dev server "${cfg.name}" did not become ready on :${port} within 30s` };
        }
