...
import { createBackgroundTask, updateBackgroundTask, listBackgroundTasks, getBackgroundTask, currentPhase, } from '@kinqs/brainrouter-core/background';
...
import { listRequirements, getRequirement, createRequirement, updateRequirement, deleteRequirement, } from '@kinqs/brainrouter-core/requirement';
...
import { listInstalledPlugins, searchRegistryPlugins, pluginConsentSummary, installPluginFromRegistry, installPluginFromSource, setPluginEnabledBridge, setPluginConsentBridge, removePluginBridge, } from './pluginBridge.js';
...
import { listAnnotations, getAnnotation, createAnnotation, setStatus as setAnnotationStatus, addComment as addAnnotationComment, } from '@kinqs/brainrouter-core/annotation';
...
import { listArtifacts, createArtifact, updateArtifact, getArtifact, revertArtifact, } from '@kinqs/brainrouter-core/artifact';
...
export function buildQueries(ctx) {
    ...
    return {
        ...
        'runtime-runner-status': async (args) => {
            ...
            if (!runtimeId || !sessionKey)
                return { error: 'runtimeId and sessionKey are required.' };
        ...
        'runtime-preview-register': (args) => {
            ...
            if (!runtimeId || !name)
                return { ok: false, error: 'runtimeId and name are required.' };
            try {
        ...
        'runtime-preview-remove': (args) => ({
            ok: removeRuntimePreviewPort(workspaceRoot, typeof args.runtimeId === 'string' ? args.runtimeId : '', typeof args.name === 'string' ? args.name : ''),
        }),
        ...
        'plugin-search': async (args) => {
            ...
            const friendly = (raw) => (/HTTP \d+|fetch failed|ENOTFOUND|ECONNREFUSED|ETIMEDOUT/i.test(raw)
            ...
            try {
                ...
                if (!res.ok)
                    return { ok: false, hits: [], error: friendly(res.error) };
        ...
        'plugin-consent': async (args) => {
            try {
                ...
                if (res.ok)
                    return { ok: true, summary: res.summary, action, scope };
                return res;
            }
            catch (err) {
                return { ok: false, error: err instanceof Error ? err.message : String(err) };
        ...
        'action:plugin-install': (args) => {
            try {
                ...
                const name = typeof args.name === 'string' ? args.name.trim() : '';
                if (!name)
                    return { ok: false, error: 'plugin name or source is required' };
        ...
        'git-pr': async () => {
            ...
            if (cachedPr && now - cachedPr.at < 60_000)
                return { pr: cachedPr.pr };
        ...
        'git-pr-status-map': async () => {
            ...
            if (cachedPrMap && now - cachedPrMap.at < 60_000)
                return { prs: cachedPrMap.prs };
        ...
        'annotation-create': async (a) => {
            ...
            const input = { type, body, sessionKey: getActiveAgent()?.sessionKey };
        ...
        'artifact-create': async (a) => {
            ...
            const input = { kind: a.kind, title, sessionKey: getActiveAgent()?.sessionKey };
        ...
        'config-snapshot': async () => {
            ...
            return {
                ...
                triggerSecretsSet: (() => {
                    const t = (cli?.triggers ?? {});
                    const isSet = (k) => typeof t[k] === 'string' && t[k].length > 0;
        ...
        'goal-continuation': () => {
            ...
            if (!goal)
                return { action: 'none' };
            const lastTurnToolCalls = getActiveAgent().lastTurnToolCalls ?? 0;
            const lastGoalTransition = getActiveAgent().lastGoalTransition;
        ...
        'plan-record-decision': async (a) => {
            ...
            try {
                const memoryId = await emitAgentEvent({ mcpClient, sessionKey: getActiveAgent().sessionKey }, {
                    kind: 'agent_output',
                ...
                if (memoryId)
                    linkPlanDecision(workspaceRoot, getActiveAgent().sessionKey, decision.id, memoryId);
            }
        ...
        'action:set-cli-json': (args) => {
            ...
            const prevTriggers = fresh.cli?.triggers;
        ...
        'action:set-cli-schema-knob': (args) => {
            ...
            if (!field)
                return { ok: false, error: 'Unknown schema field.' };
            const fresh = loadConfig();
            const cli = (fresh.cli = fresh.cli ?? {});
        ...
        'action:runtime-remove-record': (args) => {
            ...
            if (!id)
                return { ok: false, error: 'No runtime id.' };
            try {
                removeRuntimeRecord(workspaceRoot, id);
                return { ok: true, id };
            }
            catch (err) {
        ...
        'action:runtime-resume-archive': (args) => {
            const id = typeof args.id === 'string' ? args.id.trim() : '';
            if (!id)
                return { ok: false, error: 'No archive id.' };
            try {
        ...
        'action:runtime-prune-archives': (args) => {
            ...
            try {
                const removed = pruneArchives(keepN === undefined ? {} : { keepN });
                return { ok: true, removed };
            }
            catch (err) {
                return { ok: false, error: err instanceof Error ? err.message : 'prune failed' };
            }
        },
        'action:automation-rule-enabled': (args) => {
            ...
            if (!id)
                return { ok: false, error: 'No rule id.' };
        ...
        'action:set-provider': (args) => {
            ...
            const cachedModels = Array.isArray(args.cachedModels)
                ? [...new Set(args.cachedModels.filter((m) => typeof m === 'string').map((m) => m.trim()).filter(Boolean))]
        ...
        'browser:suggest-stories': async () => {
            try {
                ...
                const resp = await callOpenAI(llm, [{ role: 'system', content: p.system }, { role: 'user', content: p.user }], [{ name: p.toolName, description: p.toolDescription, inputSchema: p.toolSchema }], { effort: 'low', tool_choice: { type: 'function', function: { name: p.toolName } } });
                const argsText = resp?.tool_calls?.[0]?.function?.arguments;
        ...
        'browser:run-report': async (args) => {
            ...
            const out = browser.runReport({
                ...
                screenshots: Array.isArray(args.screenshots) ? args.screenshots : [],
            });
            if (out.error || !out.reportPath)
                return out;
            try {
        ...
        'design:read-canvas-document': () => {
            try {
                return design.readCanvasDocument();
            }
            catch (err) {
                return { error: err instanceof Error ? err.message : String(err) };
            }
        ...
        'design:write-canvas-document': (args) => {
            try {
                return design.writeCanvasDocument(args.document);
            }
            catch (err) {
                return { ok: false, error: err instanceof Error ? err.message : String(err) };
            }
        ...
        'design:quick-generate': async (args) => {
            ...
            if (!prompt)
                return { html: '', error: 'Describe the component first.' };
            const llm = llmForSession(getActiveAgent().sessionKey);
            if (!llm || (!llm.apiKey && (llm.provider ?? 'openai') === 'openai')) {
            ...
            try {
                ...
                const raw = (resp?.content ?? '').trim().replace(/^```[a-zA-Z]*\n?/, '').replace(/\n?```$/, '').trim();
                ...
                if (/<script\b/i.test(raw))
                    return { html: '', error: 'The model returned a script tag, which is not allowed in a component.' };
        ...
        'design:read-brand-overrides': () => {
            try {
                return design.readBrandOverrides();
            }
            catch (err) {
                return { error: err instanceof Error ? err.message : String(err) };
            }
        ...
        'design:write-brand-overrides': (args) => {
            try {
                return design.writeBrandOverrides(args.overrides);
            }
            catch (err) {
                return { ok: false, error: err instanceof Error ? err.message : String(err) };
            }
        ...
        'action:add-mcp': async (args) => {
            ...
            saveConfig(fresh);
            try {
                await mcpClient.connectOne(id, cfg, loadConfig().llm ?? getLlm(), 5_000);
            }
