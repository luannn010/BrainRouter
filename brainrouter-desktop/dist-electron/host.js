...
import { createComputerUseBridge, createSecretBridge, git } from './host/helpers.js';
...
import { loadConfig, saveConfig } from '@kinqs/brainrouter-core/config';
import { McpClientPool } from '@kinqs/brainrouter-core/mcp';
import { loadTranscript, transcriptExists, appendTranscriptEntry, getSessionRuntime, setSessionRuntime, resolveSessionLlmConfig, } from '@kinqs/brainrouter-core/session';
...
import { hashDiff, reviewGate, staleIfDiffChanged, } from '@kinqs/brainrouter-core/review';
...
import { createBackgroundTask, updateBackgroundTask, appendTaskProgress, linkBackgroundTaskMemory, currentPhase, reconcileBackgroundTasks, } from '@kinqs/brainrouter-core/background';
...
import { recordPlanDecision, linkPlanDecision } from '@kinqs/brainrouter-core/task';
...
import { buildBaseGraph, readAtlasGraph, carryForwardSummaries, buildAtlasChangeContext, } from '@kinqs/brainrouter-core/atlas';
...
async function main() {
    ...
    const runReview = async (ctx) => {
        ...
        try {
            atlasGraph = carryForwardSummaries(buildBaseGraph(workspaceRoot), atlasGraph);
        }
        catch { /* keep the stored graph as-is if a rebuild isn't possible */ }
        ...
        if (visible.length > 40) {
            try {
                ...
                if (existing)
                    updateArtifact(workspaceRoot, existing.id, { content: doc, title, status: 'draft' });
                else
                    createArtifact(workspaceRoot, { kind: 'markdown-report', title, content: doc, format: 'markdown', status: 'draft', path: UNDERSTANDING_PATH });
            }
            ...
            try {
                ...
                if (result && Array.isArray(result.items) && result.items.length > 0) {
                    ...
                    for (const n of stepNotes) {
                        try {
                            setAnnotationStatus(workspaceRoot, n.id, 'resolved');
                        }
                        catch { /* advisory */ }
                    }
    ...
    const { ghText, ghJson, githubApiBase, githubStaticToken, githubConnectorToken, connectorEnvToken, mcpConnectorClient, githubTokenJson, currentGitBranch, trackItemForBranch, readTrackPrStatus, githubGhValidationClient, githubTokenValidationClient, githubGhPermissionClient, githubTokenPermissionClient, validateGithubConnector, githubConnectorClient, indexConnectorMemory, runConnector, syncConnectorPermissions, createTrackDraftPr, importTrackIssuesFromGh, mergeCurrentTrackPr, submitTrackPrReview, fixCurrentTrackPrChecks, connectorSchedulerTimer, connectorSchedulerBootTimer, resetGhEnvCache, } = githubTrackServices;
